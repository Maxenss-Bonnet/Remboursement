...
class ResoumissionConstatDialog(ctk.CTkToplevel, TaskRunnerMixin, AnimationMixin):
    ...
    def _build_ui(self, demande):
        ...
        button_frame.grid(row=1, column=0, columnspan=2, pady=(0, 20), sticky="s")
        self.btn_submit_corr = ctk.CTkButton(button_frame, text="Resoumettre le Constat",
                                             command=self._submit_correction_constat)
        self.btn_submit_corr.pack(side="left", padx=10)
        self.btn_renvoyer = ctk.CTkButton(button_frame, text="Renvoyer au Demandeur",
                                          command=self._reject_and_return_to_demandeur,
                                          fg_color="#D35400", hover_color="#A84300")
        self.btn_renvoyer.pack(side="left", padx=10)

        ctk.CTkLabel(form_frame, text="Veuillez fournir une nouvelle preuve et un commentaire.").pack(
            pady=(10, 15),
            padx=10)
        ...
        self.cb_keep_pj = ctk.CTkCheckBox(form_frame, variable=self.keep_pj_var, command=self._toggle_pj_ui)
        if pjs_existantes:
        ...
        ctk.CTkLabel(form_frame, text="Commentaire de correction (Obligatoire):").pack(pady=(10, 0),
                                                                                       padx=10)
        self.commentaire_box = ctk.CTkTextbox(form_frame)
        self.commentaire_box.pack(pady=(5, 10), padx=10, fill="both", expand=True)
    ...
    def _toggle_pj_ui(self):
        ...
        if is_kept:
            self.preview_title_label.configure(text="Aperçu")
            self.preview_image_label.configure(image=None, text="Sélectionnez un fichier\npour voir un aperçu.")
    ...
    def _submit_correction_constat(self):
        ...
        if not self.keep_pj_var.get() and not self.chemin_pj_reseau:
            self.app_controller.show_toast(
                "Une nouvelle preuve est obligatoire si vous ne conservez pas l'ancienne.", "error")
        ...
        def task():
            ...
            self.chemin_pj_reseau = None
            return self.remboursement_controller.mlupo_resoumettre_constat_corrige(self.id_demande,
                                                                                   commentaire,
    ...
    def _reject_and_return_to_demandeur(self):
        dialog = CommentDialog(self, title="Renvoyer au Demandeur",
                               prompt="Motif du renvoi à p.neri (obligatoire) :", is_mandatory=True)
